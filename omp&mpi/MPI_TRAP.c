...
int main(void){
	...
	MPI_Init(NULL,NULL);
	MPI_Comm_rank(MPI_COMM_WORLD,&my_rank);//current
	MPI_Comm_size(MPI_COMM_WORLD,&comm_sz);//total
	int map[comm_sz];
	for(i=0;i<comm_sz;i++){
		map[i] = 1;
	}
	...
	for(step = 4;step<comm_sz;step*=2){
		if(my_rank % step !=0){
			MPI_Send(&total_int,1,MPI_DOUBLE,my_rank-step/2,0,MPI_COMM_WORLD);
		}
		else{
			MPI_Recv(&local_int,1,MPI_DOUBLE,my_rank+step/2,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			//printf("rank %d local %f total %f\n",my_rank,local_int,total_int);
			total_int += local_int;
		}
	}
	...
	if(my_rank == 0){
		MPI_Recv(&local_int,1,MPI_DOUBLE,comm_sz/2,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		total_int += local_int;
	}
